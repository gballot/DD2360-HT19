 ...
 */
__global__ void gpu_grayscale(int width, int height, float *image,
    float *image_out)
{
 ...
 */
__device__ float gpu_applyFilter(float *image, int stride, float *matrix,
    int filter_dim)
{
  // We need a second function to be called from the device even if it's
  // the same code.
 ...
 */
__global__ void gpu_gaussian(int width, int height, float *image,
    float *image_out)
{
  float gaussian[9] = { 1.0f / 16.0f, 2.0f / 16.0f, 1.0f / 16.0f,
    2.0f / 16.0f, 4.0f / 16.0f, 2.0f / 16.0f,
